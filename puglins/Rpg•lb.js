...
let sortedLim = users.map(toNumber('hcoin')).sort(sort('hcoin'))
...
││ ${sortedLim.slice(0, len).map(({ jid, hcoin }, i) => `${i + 1}. ${participants.some(p => jid === p.jid) ? `(${conn.getName(jid)}) wa.me/` : '@'}${jid.split`@`[0]} *${hcoin} 💰*`).join`\n││ `}
│╰────────────────···
╰───────────═┅═──────────
