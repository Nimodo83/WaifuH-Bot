...
return;
  ...
  if (users.hcoin < rob) return conn.reply(m.chat, `😔 @${who.split`@`[0]} Tiene menos de *${ro} H-Coins*\nNo robes a un pobre :v`, m, {mentions: [who]});
  global.db.data.users[m.sender].hcoin += rob;
  global.db.data.users[who].hcoin -= rob;
  conn.reply(m.chat, `*🚩 Robastes ${rob} H-Coins a @${who.split`@`[0]}*`, m, {mentions: [who]});
